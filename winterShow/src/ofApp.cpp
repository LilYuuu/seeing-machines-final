...
//    cout << "kinect2 : " << kinects[1].getSerial() << endl;
    ...
    projectorFbo.allocate(ofGetWindowWidth(), ofGetWindowHeight());
}
...
//        cout << "contourFinder_ceiling  WIDTH:" << contourFinder_ceiling.getWidth()<< " HEIGHT:" << contourFinder_ceiling.getHeight() << endl ;
    ...
    /* --------------- SOUND --------------- */
    currElapsedTime = ofGetElapsedTimef();
    ...
    if (currElapsedTime - timer >= 60) {
        clockSound.stop();
        isExecuted = false;
    }
    
    if (contourFinder_ceiling.nBlobs == 0) {
        if (currElapsedTime - timer > 3) {
            clockSound.stop();
            isExecuted = false;
        }
    }
...
//
//        /* ----- shadow ----- */
//        drawShadowOnClock("second", oriSecHandRad, shadowFbo);
...
//    thresholdImg_ceiling.draw(0,0);
    ...
    if (contourFinder_ceiling.nBlobs > 0) {
        ...
        /* ----- shadow ----- */
        drawShadowOnClock("second", oriSecHandRad, shadowFbo, 1.0);
        drawShadowOnClock("minute", minHandRad, shadowFbo, 1.0);
        drawShadowOnClock("hour", hrHandRad, shadowFbo, 1.0);
...
//--------------------------------------------------------------
void ofApp::drawShadowOnClock(string hand, float rotation, ofFbo fbo, float scale){
...
//    ofTranslate(ofGetWindowWidth()/2, ofGetWindowHeight()/2);
//    if (currElapsedTime - timer <= 50) {
//        projectorFbo.draw(-ofGetWindowWidth()/2, -ofGetWindowHeight()/2);
//    } else {
//        projectorFbo.draw(-ofGetWindowWidth()/2, -ofGetWindowHeight()/2, ofGetWindowWidth() * (60 - (currElapsedTime - timer))/10, ofGetWindowHeight() * (60 - (currElapsedTime - timer))/10);
//    }
    
    
    if (contourFinder_ceiling.nBlobs > 0) {

        if (currElapsedTime - timer < 59) {
            ofPushMatrix();
            ...
            if (currElapsedTime - timer < 50) {
                clockRad = ofGetWindowHeight()/2;
            } else {
                clockRad = ofLerp(clockRad, ofGetWindowHeight()/2 * (60 - (currElapsedTime - timer)) / 10, lerpSpeed);
//                hrShadowScale = ofLerp(hrShadowScale, hrShadowScale * (60 - (currElapsedTime - timer)) / 10, lerpSpeed);
//                minShadowScale = ofLerp(minShadowScale, minShadowScale * (60 - (currElapsedTime - timer)) / 10, lerpSpeed);
//                secShadowScale = ofLerp(secShadowScale, secShadowScale * (60 - (currElapsedTime - timer)) / 10, lerpSpeed);
            }
            
            ofDrawCircle(ofGetWindowWidth()/2, ofGetWindowHeight()/2, clockRad);
            ...
            /* ----- shadow ----- */
//            if (currElapsedTime - timer >= 50) {
//                oriSecHandRad = ofLerp(oriSecHandRad, oriSecHandRad * (60 - (currElapsedTime - timer)) / 10, lerpSpeed);
//                minHandRad = ofLerp(minHandRad, minHandRad * (60 - (currElapsedTime - timer)) / 10, lerpSpeed);
//                hrHandRad = ofLerp(hrHandRad, hrHandRad * (60 - (currElapsedTime - timer)) / 10, lerpSpeed);
//            }
            drawShadowOnClock("second", oriSecHandRad, shadowFbo, secShadowScale);
            drawShadowOnClock("minute", minHandRad, shadowFbo, minShadowScale);
            drawShadowOnClock("hour", hrHandRad, shadowFbo, hrShadowScale);
