...
void ofApp::setup(){
    ...
    farThreshold.set("Far Threshold", 0.037f, 0.0f, 0.1f);
    ...
    /* ----- kinect2 ----- */
    // kinect2 , the one to get the outline of user
    ...
    /* ----- kinect1 ----- */
    //kinect1 , the one on the ceiling
    ...
    colorImg2.allocate(kinect1.getWidth(), kinect1.getHeight());
    // Fbo img initialization
    maskFbo2.allocate(ofGetWindowWidth(), ofGetWindowHeight(), GL_RGBA);
    
    // Setup the parameters for kinect1.
    nearThreshold2.set("Near Threshold2", 0.0001f, 0.0f, 0.1f);
    farThreshold2.set("Far Threshold2", 0.0265f, 0.0f, 0.1f);
    
    // Add kinect1 to the gui.
    gui.add(nearThreshold2);
    gui.add(farThreshold2);
    

    
    cout << "kinect2 : " << kinect2.getSerial() << endl;
    cout << "kinect1 : " << kinect1.getSerial() << endl;
...
void ofApp::update(){
    ...
    kinects[a].update();
//    cout << "kinect2 /// WIDTH:" << kinect2.getWidth() << " HEIGHT:" << kinect2.getHeight() << endl ;
...
//    cout << "mixFbo /// WIDTH:" << mixFbo.getWidth() << " mixFbo HEIGHT:" << mixFbo.getHeight() << endl ;
    ...
    // draw the contour & Blob on maskFbo ( kinect2's fbo)
    maskFbo.begin();
    ofClear(255, 255, 255);
    if (contour.nBlobs > 0) {
        ofPushMatrix();
...
//        cout << "contourFinder2  WIDTH:" << contourFinder2.getWidth()<< " HEIGHT:" << contourFinder2.getHeight() << endl ;
    ...
    // draw the cameraView + contour & Blob on maskFbo2 ( kinect1's fbo)
    maskFbo2.begin();
    ofClear(255, 255, 255);
    colorImg2.draw(0, 0);
...
void ofApp::keyPressed (int key) {
    switch (key) {
        case OF_KEY_LEFT:
            ...
            kinects[b].setCameraTiltAngle(angle);
            break;
...
//        #ifdef USE_TWO_KINECTS
        ...
        case OF_KEY_UP:
            ...
            kinects[a].setCameraTiltAngle(angle2);
            break;
